...
function MovieCard({movieObj, watchList, addToWatchList, removeFromWatchList}) {
    ...
    //checking if a particular movie is present in watchlist or not
  ...
  return (
    <div
      ...
      <div className="text-white w-full text-center text-xl p-2 bg-gray-900/70 rounded-xl flex-end">{movieObj.title}</div>
      {
        doesContain() ? 
        (<div className="flex justify-center m-4 h-8 w-8 items-center rounded-lg bg-gray-900/70"
            onClick={() => removeFromWatchList(movieObj)}>❌</div>) 
        : (<div className="flex justify-center m-4 h-8 w-8 items-center rounded-lg bg-gray-900/70"
            onClick={() => addToWatchList(movieObj)}>😍</div>)
        }
    </div>
  );
}
